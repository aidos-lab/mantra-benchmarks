...
from .cell_mp import CochainMessagePassing, CochainMessagePassingParams
...
class DummyCochainMessagePassing(CochainMessagePassing):
    ...
    def __init__(
        ...
        super(DummyCochainMessagePassing, self).__init__(
            up_msg_size,
            down_msg_size,
    ...
    def forward(self, cochain: CochainMessagePassingParams):
        up_out, down_out, boundary_out = self.propagate(
            cochain.up_index,
            cochain.down_index,
...
class DummyCellularMessagePassing(torch.nn.Module):
    def __init__(
        ...
        for dim in range(max_dim + 1):
            mp = DummyCochainMessagePassing(
                input_dim,
                input_dim,
                boundary_msg_size=input_dim,
                use_boundary_msg=use_boundary_msg,
                use_down_msg=use_down_msg,
            )
...
class CINCochainConv(CochainMessagePassing):
    ...
    def __init__(
        ...
        super(CINCochainConv, self).__init__(
            up_msg_size, down_msg_size, use_boundary_msg=False
        )
    ...
    def forward(self, cochain: CochainMessagePassingParams):
        out_up, out_down, _ = self.propagate(
            cochain.up_index,
            cochain.down_index,
            None,
            x=cochain.x,
...
class EdgeCINConv(torch.nn.Module):
    ...
    def __init__(
        ...
        up_msg_size: int,
        down_msg_size: int,
        v_msg_up_nn: Callable,
        e_msg_down_nn: Callable,
        e_msg_up_nn: Callable,
        v_update_nn: Callable,
        e_update_nn: Callable,
        eps: float = 0.0,
        train_eps=False,
    ):
...
class SparseCINCochainConv(CochainMessagePassing):
    ...
    def forward(self, cochain: CochainMessagePassingParams):
        out_up, _, out_boundaries = self.propagate(
            cochain.up_index,
            cochain.down_index,
            cochain.boundary_index,
            x=cochain.x,
...
class CINppCochainConv(SparseCINCochainConv):
    ...
    def __init__(
        ...
        up_msg_size: int,
        down_msg_size: int,
        boundary_msg_size: int,
        msg_up_nn: Callable[..., Any],
        msg_boundaries_nn: Callable[..., Any],
        msg_down_nn: Callable[..., Any],
        update_up_nn: Callable[..., Any],
        update_boundaries_nn: Callable[..., Any],
        update_down_nn: Callable[..., Any],
        combine_nn: Callable[..., Any],
        eps: float = 0,
        train_eps: bool = False,
    ):
        super(CINppCochainConv, self).__init__(
            dim,
            up_msg_size,
            down_msg_size,
            boundary_msg_size,
    ...
    def forward(self, cochain: CochainMessagePassingParams):
        ...
        # Because the cross product of countable spaces is countable, then such a function exists.
        # And we can learn it with another MLP.
        return self.combine_nn(
            torch.cat([out_up, out_down, out_boundaries], dim=-1)
        )
...
class SparseCINConv(torch.nn.Module):
    ...
    def forward(
        self, *cochain_params: CochainMessagePassingParams, start_to_process=0
    ):
        assert len(cochain_params) <= self.max_dim + 1
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(CINppConv, self).__init__(
            ...
            boundary_msg_size,
            passed_msg_up_nn,
            passed_msg_boundaries_nn,
            passed_update_up_nn,
            passed_update_boundaries_nn,
            eps,
            train_eps,
            max_dim,
            graph_norm,
        ...
        for dim in range(max_dim + 1):
            ...
            mp = CINppCochainConv(
                ...
                boundary_msg_size=boundary_msg_size,
                msg_up_nn=msg_up_nn,
                msg_down_nn=msg_down_nn,
                msg_boundaries_nn=msg_boundaries_nn,
                update_up_nn=update_up_nn,
                update_down_nn=update_down_nn,
                update_boundaries_nn=update_boundaries_nn,
                combine_nn=combine_nn,
                eps=eps,
                train_eps=train_eps,
...
class OrientedConv(CochainMessagePassing):
    def __init__(
        ...
        up_msg_size: int,
        down_msg_size: int,
        update_up_nn: Optional[Callable],
        update_down_nn: Optional[Callable],
        update_nn: Optional[Callable],
        act_fn,
        orient=True,
    ):
    ...
    def forward(self, cochain: Cochain):
        ...
        out_up, out_down, _ = self.propagate(
            ...
            None,
            x=cochain.x,
            up_attr=cochain.upper_orient.view(-1, 1),
            down_attr=cochain.lower_orient.view(-1, 1),
        )
...
class AbstractEmbedVEWithReduce(torch.nn.Module, ABC):
    ...
    def forward(self, *cochain_params: CochainMessagePassingParams):
        ...
        if c_params is not None:
            ...
            cx = self.init_reduce(reduced_ex, c_params.boundary_index) / 2.0
            out.append(cx)
...
class EmbedVEWithReduce(AbstractEmbedVEWithReduce):
    ...
    def __init__(
        ...
        super(EmbedVEWithReduce, self).__init__(
            v_embed_layer, e_embed_layer, init_reduce
        )
...
class OGBEmbedVEWithReduce(AbstractEmbedVEWithReduce):
    ...
    def __init__(
        ...
        super(OGBEmbedVEWithReduce, self).__init__(
            v_embed_layer, e_embed_layer, init_reduce
        )
