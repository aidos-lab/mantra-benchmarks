...
from models.cells.transformer.positional_encodings.RandomWalks import generate_pe_from_transition_matrix
...
def barycentric_subdivision_random_walk_positional_encoding(
    ...
    if isinstance(t_complex, CellComplex):
        G, map_cells_graph_vertices = barycentric_subdivision_1_skeleton_cell_complex(
            t_complex
        )
    else:
        G, map_cells_graph_vertices = (
...
def barycentric_subdivision_1_skeleton_cell_complex(c_complex: CellComplex):
    ...
    map_cells_graph_vertices = {
        cell: graph_vertex for graph_vertex, cell in enumerate(nodes + edges + cells)
    }
    graph = nx.Graph()
    graph.add_nodes_from(range(len(map_cells_graph_vertices)))
    # Add labels to the nodes corresponding to the cells
    nx.set_node_attributes(
        graph, {i: cell for i, cell in enumerate(nodes + edges + cells)}, "cell"
    )
    for edge in edges:
        graph.add_edge(
    ...
    for cell in cells:
        for vertex in cell.elements:
            graph.add_edge(
                map_cells_graph_vertices[cell], map_cells_graph_vertices[vertex]
            )
...
def barycentric_subdivision_1_skeleton_simplicial_complex(s_complex: SimplicialComplex):
    ...
    for simplex in simplices:
        for face in chain.from_iterable(
            ...
            G.add_edge(
                map_cells_graph_vertices[simplex], map_cells_graph_vertices[face_as_set]
            )
...
class BarycentricSubdivisionRandomWalkPE(BasePositionalEncodings):
    def generate_positional_encodings(
        ...
        pe = {i: torch.tensor(pes[i], dtype=torch.float32) for i in range(len(pes))}
        return pe
