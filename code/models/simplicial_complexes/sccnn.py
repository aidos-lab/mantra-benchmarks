...
class SCCNN(nn.Module):
    ...
    def __init__(self, config: SCCNNConfig):
        super().__init__()
        ...
        self.readout_0 = SumReadout(
            config.hidden_channels_all[0], config.out_channels
        )
        self.readout_1 = SumReadout(
            config.hidden_channels_all[1], config.out_channels
        )
        self.readout_2 = SumReadout(
            config.hidden_channels_all[2], config.out_channels
        )
    ...
    def forward(self, batch):
        ...
        incidence_all = (
            connectivity_matrices["incidence_1"],
            connectivity_matrices["incidence_2"],
        )

        x_0, x_1, x_2 = self.sccnn_backbone(
            x_all, laplacian_all, incidence_all
        )
